...
def pesquisa_postagem():
    ...
    if request.method == "POST":
        ...
        if cur.rowcount > 0:# se existir esta postagem
            ...
            cur.close()
            print (Posts)
           return render_template('posts.html', id_canal=id_canal,Posts=posts, canais=getcanais(id_usuario), titulocanal=getChannel(id_canal), seguidor=seguidor, pode_criar_canal = pode_criar_canais(recuperar_id_usuario_logado()), pode_gerenciar_usuario = pode_gerenciar_usuarios(recuperar_id_usuario_logado()))
